...
class Ant {
	private:
		...
		void propagate(float input[NW], float output[NW]) {
			...
			for (int layer=0;layer<LC;layer++) {
				...
				for (int neuron=0;neuron<NW;neuron++) {
					...
					new_activations[neuron] = a;//sigmoid(a);
				}
		...
		void calculate_attraction(float atrc[4]) {
			...
			for (int i=0;i<FOODCOUNT;i++) {
				...
				float d = powf(pos[0]-food[i][0], 2)+powf(pos[1]-food[i][1], 2);
	...
	public:
		...
		Ant(float nn_template[LC][NW][NW]) {
			...
			memset(nn_delta, 0, sizeof(nn_delta));
				...
				for (int n=0;n<NW;n++) {
					for (int w=0;w<NW;w++) {
						if (rand() % 20 == 0) {
							...
							nn[l][n][w] += dw*mutation;
							nn_delta[l][n][w] = dw*mutation;
						}
		...
		void update() {
				...
				for (int i=0;i<4;i++) { memory[i] = sigmoid(output[i+4]);}
				
				//cout << output[0] << "|" << output[1] << "|" << output[2] << "|" << output[3] << "\n";
				
				float vx = (output[3]-output[2]);  // sigmoid(inf) == 1  !!!!
...
void write_parsed_nn(float nn[LC][NW][NW]) {
	
	ofstream outfile("changelog.pnn", ios_base::out | ios_base::app);
	cout << "Writing delta nn\n";
	...
	while (1==1) {
		...
		if (COMBINATE_NN) {
			...
			for (list<Ant>::iterator it=ants.begin();it != ants.end(); it++) {
				...
				float nn_median[LC][NW][NW] = {0};
				for (int l=0;l<LC;l++) {
					for (int n=0;n<NW;n++) {
						for (int w=0;w<NW;w++) {
							nn_median[l][n][w] = (it->nn[l][n][w]+cnn[l][n][w]) / 2;
						}
					}
				}
